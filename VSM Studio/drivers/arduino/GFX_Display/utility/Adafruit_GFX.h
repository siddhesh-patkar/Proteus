...
class Adafruit_GFX : public Print {
 public:
	...
	// This is necessary because the compiler is flunking something.
  ...
  */
  ...
  void image(PImage & img, int16_t x, int16_t y);
#endif
...
void Adafruit_GFX::image(PImage & img, int16_t x, int16_t y) {
  int16_t w, h, row, col;
  uint8_t  r, g, b;
  uint32_t pos = 0;
...
uint16_t PImage::read16(SDLib::File f) {
  uint16_t result;
...
uint32_t PImage::read32(SDLib::File f) {
  uint32_t result;
  ((uint8_t *)&result)[0] = f.read(); // LSB
